 ...
 *
 */
public class SearchRelStepAction extends JDialog implements ActionListener {
...
//	private JLabel readLabel;
	...
	public SearchRelStepAction(StepInfoPane pane) {
		super();
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		this.setTitle(stepInfo.getInfo().getStepno() + "步骤可能存在关联关系的步骤");
	...
	private void init() {
		...
		this.readLabel = new JLabel("本数据源关联步骤:",JLabel.CENTER);
//		this.readLabel.set
		this.updateLabel    = new JLabel("其他数据源关联步骤:",JLabel.CENTER);
...
//		cons.gridwidth = 5;
	...
	public void setInfo(StepInfo stepInfo){
		...
		try {
			relStep = new RelStep();
			rlist = relStep.relStepFilteredSep(info.getStepno());
		} catch (Exception e) {
			// TODO 自动生成 catch 块
			e.printStackTrace();
		}
		...
		if (rlist != null) {
			...
			RelStepBean step = null;
			String lineSep = System.getProperty("line.separator", "\r\n");
			for (int i = 0; i < rlist.get(0).size() - 1; ++i) {
				step = rlist.get(0).get(i);
				str.append(step.getStepno() + ", " + step.getTbname() + lineSep);
			}
			if (rlist.get(0).size() > 0) {
				step = rlist.get(0).get(rlist.get(0).size() - 1);
				str.append(step.getStepno() + ", " + step.getTbname());
			} else {
				str.append("无");
			}
...
//			System.out.println("更新的表：");
			...
			for (int i = 0; i < rlist.get(1).size() - 1; ++i) {
				step = rlist.get(1).get(i);
				str.append(step.getStepno() + ", " + step.getTbname() + lineSep);
			}
			if (rlist.get(1).size() > 0) {
				step = rlist.get(1).get(rlist.get(1).size() - 1);
				str.append(step.getStepno() + ", " + step.getTbname());
			}else {
				str.append("无");
			}
			...
			this.readArea.setText("无");
			this.updateArea.setText("无");
		}
	}
